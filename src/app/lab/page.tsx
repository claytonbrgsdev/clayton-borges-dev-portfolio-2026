...
function MotifForm({ color }: { color: string }) {
  // Hypotrochoid trace — Kikai aesthetic
  const pts: string[] = [];
  const R = 14, r = 5, d = 9;
...
function MotifForce({ color }: { color: string }) {
  // Lorenz attractor suggestion — two looping wings
  return (
    <svg width="40" height="40" viewBox="0 0 40 40" fill="none">
...
function MotifMind({ color }: { color: string }) {
  // Sierpinski triangle — three nested triangles
  return (
    <svg width="40" height="40" viewBox="0 0 40 40" fill="none">
...
function PartCard({
  ...
  onHover: () => void;
  onLeave: () => void;
  lineReveal: number; // 0→1, drives the line animation
}) {
  const Motif =
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        {/* Line segment going down (except last) */}
        {part.number < 3 && (
          <div
            style={{
              width: 1,
...
// ── Animated background canvas (subtle particle field) ─────────────────────────
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030304", color: "#fff", position: "relative" }}>
      ...
      >
        ...
        {/* Roadmap — three part cards with timeline */}
